...
class storeController
{
  ...
  public static function storeData()
  {
    ...
    if (loginController::getUserUsername()) {
      ...
      if ($_POST['activite'] == NULL) {
        ...
        foreach ($lines as $line) {
          if ($line[0] == loginController::getUserUsername()) {
            $tmp = true;
            break;
          } else {
            $tmp = false;
        ...
        if ($tmp == false) {
          ...
          $data = array(loginController::getUserUsername());
        ...
        foreach ($lines as $line) {
          ...
          if ($line[0] == loginController::getUserUsername() && $line != null && $line != '') {
            ...
            //ajout des lines déja existante qui ne sont pas de l'utilisateur connecté
              ...
              if ($line[0] != loginController::getUserUsername() && $line != null) {
                foreach ($lines as $line) {
                  fputcsv($file, $line);
                }
              } else {
      ...
      if ($numb >= 9) {
        ...
        $txt = "l'utilisateur " . loginController::getUserUsername() . " a modifier l'activé " . "$_POST[activite]" . " le " . date("Y-m-d H:i:s") . "\n";
              ...
              <div class='col-12 mx-auto'>
                  <div class='card bg-dark mt-5 '>
                      <div class='card-title bg-dark text-white mt-5'>
                          ...
                          <div class='card-body'>
                          <a href='" . wp_logout_url(home_url()) . "' class='btn btn-primary btn-block'>Retour</a>
                          </div>
                      </div>
                  </div>
              </div>
